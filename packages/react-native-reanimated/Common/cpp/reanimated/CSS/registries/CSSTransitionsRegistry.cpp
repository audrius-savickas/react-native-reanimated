...
void CSSTransitionsRegistry::updateConfigOrRun(
  ...
  // Apply config (platform routing + loop settings); run the loop only if value diffs remain.
  auto loopValueDiffs = transition->applyConfig(rt, std::move(config));
  if (loopValueDiffs.empty()) {
    return;
  }
  auto initialUpdate = transition->run(rt, loopValueDiffs, getUpdatesFromRegistry(transition->getViewTag()));
...
void CSSTransitionsRegistry::flushUpdates(UpdatesBatch &updatesBatch) {
  ...
  for (const auto viewTag : tags) {
    ...
    auto *loopTransition = transition->getLoopTransition();
    if (!loopTransition) {
      continue;
    }
    const auto updates = loopTransition->computeCurrentStyle(transition->getShadowNode());
